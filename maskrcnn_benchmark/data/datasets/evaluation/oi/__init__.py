...
def oi_evaluation(
    ...
    result_str_tmp = ''
    r50_result = eval_classic_recall(cfg, mode, groundtruths, predictions, predicate_cls_list,
                                     logger, result_str_tmp, result_dict_list_to_log)


    # transform the initial prediction into oi prediction format
    packed_results = adapt_results(groundtruths, predictions)

    result_str_tmp = ''
    result_str_tmp, result_dict = eval_rel_results(
        cfg, packed_results, predicate_cls_list, result_str_tmp, logger,
    )
    ...
    if output_folder:
        with open(os.path.join(output_folder, "evaluation_res.txt"), 'w') as f:
            f.write(result_str)
    if cfg.GLOBAL_SETTING.CHOOSE_BEST_MODEL_BY_METRIC == '_recall':
        return float(result_dict['R50'])
    elif cfg.GLOBAL_SETTING.CHOOSE_BEST_MODEL_BY_METRIC == 'w_final_score':
        return float(result_dict['w_final_score'])
    else:
        raise ValueError("invalid cfg.GLOBAL_SETTING.CHOOSE_BEST_MODEL_BY_METRIC")
