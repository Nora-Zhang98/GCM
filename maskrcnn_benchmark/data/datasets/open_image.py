...
from maskrcnn_benchmark.data.datasets.visual_genome import box_filter # , get_sample_rate
...
def get_statistics(train_data, must_overlap=True):
...
class OIDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, split, img_dir, ann_file, cate_info_file, transforms=None,
        ...
        self.ind_to_classes, self.ind_to_predicates, self.classes_to_ind, self.predicates_to_ind = load_cate_info(
            self.cate_info_file)  # contiguous 602, 31 containing __background__
    ...
    def __getitem__(self, index):
        ...
        # for SGDet, control the bbox number in an image to avoid OOM
        cut_num = 70
        if target.bbox.shape[0] > cut_num:
            print('box num: ' + str(target.bbox.shape[0]))
            target.bbox = target.bbox[:cut_num, :]
            target.extra_fields['attributes'] = target.extra_fields['attributes'][:cut_num, :]
            target.extra_fields['labels'] = target.extra_fields['labels'][:cut_num]
            target.extra_fields['relation'] = target.extra_fields['relation'][:cut_num, :cut_num]
            print('too many bbox in an image! cut done.')
            print('filename: '+ self.filenames[index])
    ...
    def get_statistics(self):
        fg_matrix, bg_matrix, _ = get_statistics(self, must_overlap=True)
