...
class ROIRelationHead(torch.nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        self.predictor_1 = make_roi_relation_predictor(cfg, feat_dim, only_vis=True, only_txt=False) # only visual player
        self.predictor_2 = make_roi_relation_predictor(cfg, feat_dim, only_vis=False, only_txt=True) # only linguistic player
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        ...
        """
        if self.training or self.cfg.MODEL.TRAIN_INFER:
        ...
        if self.use_union_box:
            union_features = self.union_feature_extractor(features, proposals, rel_labels, rel_pair_idxs)
        else:
            union_features = None
        ...
        # modality erasing predictor
        _, relation_logits_1, _ = self.predictor_1(proposals, rel_pair_idxs, rel_labels, rel_binarys,
                                                          roi_features, union_features, logger)
        _, relation_logits_2, _ = self.predictor_2(proposals, rel_pair_idxs, rel_labels, rel_binarys,
                                                          roi_features, union_features, logger)
        relation_logits_1 = cat(relation_logits_1, dim=0)
        relation_logits_2 = cat(relation_logits_2, dim=0)
        relation_logits = cat(relation_logits, dim=0)

        sh_1 = (relation_logits + relation_logits_2 - relation_logits_1) / 2
        pro_1 = sh_1 / relation_logits
        pro_2 = 1 - pro_1
        relation_logits = relation_logits - relation_logits_1 * pro_1 - relation_logits_2 * pro_2

        # for test
        if not self.training:
            relation_logits = relation_logits.split([r.shape[0] for r in rel_pair_idxs], dim=0)
            result = self.post_processor((relation_logits, refine_logits), rel_pair_idxs, proposals)
            return roi_features, result, {}
