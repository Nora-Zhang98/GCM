...
from .utils_motifs import rel_vectors, obj_edge_vectors, center_x, sort_by_score, to_onehot, get_dropout_mask, \
    nms_overlaps, encode_box_info
...
class LSTMContextPE(nn.Module):
    ...
    def __init__(self, config, obj_classes, rel_classes, in_channels, only_vis=False, only_txt=False):
        super(LSTMContextPE, self).__init__()
        self.cfg = config
        self.obj_classes = obj_classes
        ...
        elif only_vis == True: # modality erasing -- only visual player
    ...
    def forward(self, x, proposals, logger=None, all_average=False, ctx_average=False):

        if self.only_txt: # modality erasing -- only linguistic player
        ...
        obj_ctx, perm, inv_perm, ls_transposed = self.obj_ctx(obj_pre_rep, proposals, obj_labels, boxes_per_cls,
